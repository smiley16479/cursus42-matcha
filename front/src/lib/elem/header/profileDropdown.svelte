<script lang="ts">
  ...
  import { logout } from "@/service/user";
	...
	// import { Chat_c } from '@/types/chat/chat';
  let dropDownMenu: HTMLElement;
  let profilLogo: HTMLElement;
	let hidden = true;

  /** Destiné à l'acces au chat direct message ou suport (pas encore implémenté)*/
  let msgHidden = true;

	onMount(() => {
		// Inclure le script popover.js seulement côté client
/*   	  if (typeof window !== 'undefined') {
  	    const script = document.createElement('script');
  	    script.src = 'material-tailwind/scripts/popover.js';
  	    script.async = true;
    ...
    if (
        dropDownMenu && profilLogo &&
        event.target instanceof Node &&
        !profilLogo.contains(event.target) &&
...
<div bind:this={profilLogo}>
    <button on:click={()=> hidden = !hidden}>
      <img class="h-16" src="/svg.svg" alt="Matcha-Frida">
    </button>
    <!-- Dot de Notifications  Pas Encore DEV -->
<!--     <span
      class="absolute min-w-[12px] min-h-[12px] rounded-full py-1 px-1 text-xs font-medium content-[''] leading-none grid place-items-center top-[20px] right-[12px] bg-green-500 text-white border border-white"
    ></span> -->
</div>
<ul role="menu" data-popover="profile-menu" data-popover-placement="bottom" class:hidden bind:this={dropDownMenu}
  class="absolute right-0 z-10 flex min-w-[180px] flex-col gap-2 overflow-auto rounded-md border border-blue-gray-50 bg-white p-3 font-sans text-sm font-normal text-blue-gray-500 shadow-lg shadow-blue-gray-500/10 focus:outline-none">
  ...
  {#if $us.logState === LoggingState.logged}
    ...
    <button role="menuitem"
      on:click={()=> {hidden = !hidden; signOut()}}
      class="flex w-full cursor-pointer select-none items-center gap-2 rounded-md px-3 pt-[9px] pb-2 text-start leading-tight outline-none transition-all hover:bg-blue-gray-50 hover:bg-opacity-80 hover:text-blue-gray-900 focus:bg-blue-gray-50 focus:bg-opacity-80 focus:text-blue-gray-900 active:bg-blue-gray-50 active:bg-opacity-80 active:text-blue-gray-900">
