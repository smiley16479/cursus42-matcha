<script lang="ts">
  ...
  import Schedule from "@/lib/section/matcha/schedule.svelte";
	...
	let matches: Chat_c[] = $us.user.chats;
	let matchOrChat = true;
	let chat : Chat_c;
	onMount(()=> {
		// photo , nom , ndernier msg
	})
...
/* 	writable([{
  ...
  { id: 2, title: "Message reçu", message: "Pauline vous a envoyé un message." },
	...
	function viewMatchDetails(match: Chat_c) {
	  alert(`Afficher les détails pour ${match.interlocutor.userName}`);
		goto(`/app/frida/${match.interlocutor.id}`)
	...
	function viewChat(match: Chat_c) {
		...
		matchOrChat = false;
		$app.footer = false;
		// goto(`matcha/${match.interlocutor.id}`);
	}

	function deleteMatch(match: Chat_c) {
		if (confirm(`Vous êtes sur le point d'unmatch avec ${match.interlocutor.userName}\nPAS ENCORE DEV`)) {
			unlike(match.id);
				...
				{#each $us.user.chats as match, index}
					<div class={`flex items-center p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300 ${index === 0 ? 'border border-black bg-gray-200': 'bg-gray-50'}`}>
						<button class="flex flex-grow min-w-20" type="button" title="Voir profil" on:click={() => viewMatchDetails(match)} >
							{#if match.interlocutor?.pictures?.[0].filename}
								<img src={"http://localhost:3000/api/user/picture/" + match.interlocutor?.pictures[0].filename} alt={match.interlocutor?.userName} class="w-16 h-16 rounded-full object-cover mr-4">
							{:else}
						...
						<button class="flex flex-grow text-left w-full" type="button" title="Chatter" on:click={() => viewChat(match)} >
							<div class="flex-grow">
								<h2 class="hidden sm:block text-lg font-bold text-gray-900">{match.interlocutor?.userName}</h2>
								<p class="hidden md:block text-gray-600 text-sm">{match.interlocutor?.biography}</p>
							</div>
						</button>
