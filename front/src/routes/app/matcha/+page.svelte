...
/* 	writable([{
  ...
  { id: 2, title: "Message reçu", message: "Pauline vous a envoyé un message." },
	...
	function viewMatchDetails(profilId: number) {
			...
			<div class="space-y-4">
				{#each $us.user.chats as match, index}
					<div class={`flex items-center p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300 ${index === 0 ? 'border border-black bg-gray-200': 'bg-gray-50'}`}>
						<button class="flex flex-grow min-w-20" type="button" title="Voir profil" on:click={() => viewMatchDetails(match.interlocutors[0].id === $us.user.id ? match.interlocutors[1].id : match.interlocutors[0].id)} >
		...
		<div class="max-w-3xl mx-auto bg-white rounded-lg shadow mb-6">
			...
			<div class="space-y-4">
				{#each $us.user.likedBy as like, index}
					<div class={`flex items-center p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300 ${index === 0 ? 'border border-black bg-gray-200': 'bg-gray-50'}`}>
						<button class="flex flex-grow min-w-20" type="button" title="Voir profil" on:click={() => viewMatchDetails(like.likerUser.id)} >
						...
						<button class="flex flex-grow text-left w-full" type="button" title="Chatter" on:click={() => viewMatchDetails(like.likerUser.id)} >
							<div class="flex-grow">
