<script lang="ts">
  ...
  import Schedule from "@/lib/section/matcha/schedule.svelte";
	...
	onMount(()=> {
		...
		const constructMatches = $us.user.chats.map(e => {
			const profil = $us.user.likedBy.find(el => el.likerUser.id === e.interlocutor)?.likerUser;
			console.log(`profil`, profil);
			if (profil) {
				return new Chat_c(e.id, profil, e.msg);
			}
		})
		console.log(`constructMatches`, constructMatches);
		matches = constructMatches.filter((e) => e !== undefined )
	})
				...
				<div class={`flex items-center p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300 ${index === 0 ? 'border border-black bg-gray-200': 'bg-gray-50'}`}>
					<button class="flex flex-grow min-w-20" type="button" title="Voir profil" on:click={() => viewMatchDetails(match)} >
						<!-- {#if match.interlocutor.pictures[0].filename}
							<img src={match.interlocutor.pictures[0].filename} alt={match.interlocutor.userName} class="w-16 h-16 rounded-full object-cover mr-4">
            {:else} -->
