        ...
        CREATE OR REPLACE VIEW fullUsers AS (
            WITH
                ...
                user_notifications AS (
                    SELECT
                        n.userId,
                        JSON_ARRAYAGG(JSON_OBJECT("id", n.id, "date", n.createdAt, "involvedUserId", n.involvedUserId, "type", n.type, "payloadId", n.payloadId)) AS notifications
                    FROM
                        notifications n
                    GROUP BY
                        n.userId
                ...
                user_chats AS (
                    SELECT
                        uc.userId,
                        JSON_ARRAYAGG(JSON_OBJECT("id", uc.id, "user1Id", uc.otherUserId, "user2Id", uc.userId , "msg", cm.messages)) AS chats
                    FROM
